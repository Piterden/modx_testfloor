    ...
    Ext.applyIf(config, {
        ...
        , tbar: [{
            ...
            ,{
                ...
                , text: 'reload'
                , listeners: {
                    'click': {fn: this.reloads, scope: this}
                }
            }
        ]
    });
    regpay.grid.s.superclass.constructor.call(this, config)
};
Ext.extend(regpay.grid.s, MODx.grid.Grid, {
     reloads: function () {
    ...
    , changeCategory: function (btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                  xtype: 'regpay--window-change-category'
                  , record: r
                  , listeners: {
                    'success': {fn: function (r) {
                        // this.refresh();
                        var sels = this.getSelectionModel().getSelections();
                        var cat = Ext.getCmp('regpay--category-combo').lastSelectionText;
                        var s = this.getStore();
...
Ext.reg('regpay-grid-', regpay.grid.s);
...
Ext.reg('regpay--window-change-category', regpay.window.ChangeCategory);
