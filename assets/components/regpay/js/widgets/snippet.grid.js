...
regpay.grid.Snippets = function (config) {
    config = config || {};
    ...
    regpay.grid.Snippets.superclass.constructor.call(this, config)
};
Ext.extend(regpay.grid.Snippets, MODx.grid.Grid, {
    ...
    , changeCategory: function (btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                  xtype: 'regpay-snippet-window-change-category'
                  , record: r
                  , listeners: {
                    'success': {fn: function (r) {
                        // this.refresh();
                        var sels = this.getSelectionModel().getSelections();
                        var cat = Ext.getCmp('regpay-snippet-category-combo').lastSelectionText;
                        var s = this.getStore();
...
Ext.reg('regpay-grid-snippet', regpay.grid.Snippets);
...
regpay.window.ChangeCategory = function (config) {
    ...
    Ext.applyIf(config, {
        ...
        ,fields: [{
            ...
            ,id: 'regpay-snippet-category-combo'
...
Ext.reg('regpay-snippet-window-change-category', regpay.window.ChangeCategory);
